    ...
    def load_from_disk(dataset_path: str, fs=None, keep_in_memory: Optional[bool] = None) -> "Dataset":
        ...
        Args:
            ...
            keep_in_memory (:obj:`bool`, default ``None``): Whether to copy the dataset in-memory. If `None`, the
                ...
                disabled by setting to ``0`` either ``datasets.config.MAX_IN_MEMORY_DATASET_SIZE_IN_BYTES`` or the
                environment variable ``MAX_IN_MEMORY_DATASET_SIZE_IN_BYTES``, and in that case the dataset will not be
                loaded in memory.
