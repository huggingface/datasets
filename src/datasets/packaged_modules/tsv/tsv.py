...
@dataclass
class TsvConfig(datasets.BuilderConfig):
    """BuilderConfig for TSV."""

    sep: str = "\t"  # Set default separator to tab
    ...
    def pd_read_csv_kwargs(self):
        ...
        for pd_read_csv_parameter in _PANDAS_READ_CSV_NO_DEFAULT_PARAMETERS + _PANDAS_READ_CSV_DEPRECATED_PARAMETERS:
            if pd_read_csv_kwargs[pd_read_csv_parameter] == getattr(TsvConfig(), pd_read_csv_parameter):
...
class Tsv(datasets.ArrowBasedBuilder):
    BUILDER_CONFIG_CLASS = TsvConfig
    ...
    def _generate_tables(self, files):
        ...
        for file_idx, file in enumerate(itertools.chain.from_iterable(files)):
            tsv_file_reader = pd.read_csv(file, iterator=True, dtype=dtype, **self.config.pd_read_csv_kwargs)
            try:
                for batch_idx, df in enumerate(tsv_file_reader):
