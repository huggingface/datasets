...
class DummyDataGeneratorDownloadManager(DownloadManager):
    ...
    def auto_generate_dummy_data_folder(
        ...
        for src_path, relative_dst_path in zip(
            self.downloaded_paths, self.expected_dummy_paths
        ):
    ...
    def _create_dummy_data(
        ...
        if os.path.isfile(src_path):
            ...
            is_line_by_line_text_file = any(
                dst_path.endswith(extension) for extension in line_by_line_extensions
            )
            ...
            # xml file
                ...
                if xml_tag is None:
                    logger.warning(
                        "Found xml file but 'xml_tag' is set to None. Please provide --xml_tag"
                    )
                else:
                    Path(dst_path).parent.mkdir(exist_ok=True, parents=True)
                    with open(src_path, "r", encoding=encoding) as src_file:
                        ...
                        ), f"couldn't find xml tag {xml_tag}"
                        # remove all the elements except the first n
                        current = 0
                        for parent in tree.iter(parent_tag):
                            children = [
                                child for child in parent if child.tag == xml_tag
                            ]
                ...
                for name in files:
                    if not name.startswith("."):  # ignore files like .DS_Store etc.
                        ...
                        dst_file_path = os.path.join(
                            dst_path, Path(src_file_path).relative_to(src_path)
                        )
                        total += self._create_dummy_data(
                            src_file_path,
                            dst_file_path,
    ...
    def compress_autogenerated_dummy_data(self, path_to_dataset):
        root_dir = os.path.join(
            path_to_dataset, self.mock_download_manager.dummy_data_folder
        )
...
class DummyDataCommand(BaseTransformersCLICommand):
    ...
    def _autogenerate_dummy_data(
        self, dataset_builder, mock_dl_manager, keep_uncompressed
    ) -> Optional[bool]:
        ...
        if not keep_uncompressed:
            path_do_dataset = os.path.join(
                mock_dl_manager.datasets_scripts_dir, mock_dl_manager.dataset_name
            )
            ...
            try:
                ...
                for split_generator in split_generators:
                    ...
                    ] = split_generator.split_info.num_examples
            except OSError as e:
                logger.error(
                ...
                if all(n_examples > 0 for n_examples in n_examples_per_split.values()):
                    ...
                    empty_splits = [
                        split_name
                        for split_name in n_examples_per_split
    ...
    def _print_dummy_data_instructions(self, dataset_builder, mock_dl_manager):
        dummy_data_folder = os.path.join(
            self._path_to_dataset, mock_dl_manager.dummy_data_folder
        )
        ...
        for split in generator_splits:
            ...
            try:
                ...
                config_string = (
                    f"config {config.name} of " if config is not None else ""
                )
                dummy_data_guidance_print += (
        ...
        if len(files_to_create) > 0:
            ...
            if (
                len(files_to_create) == 1
                and next(iter(files_to_create)) == dummy_file_name
            ):
