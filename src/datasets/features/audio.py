...
class Audio:
    ...
    def encode_example(
        self, value: Union[str, bytes, bytearray, dict, "AudioDecoder"]
    ) -> dict:
        ...
        if isinstance(value, str):
            ...
            buffer = BytesIO()
            AudioEncoder(
                torch.from_numpy(value["array"].astype(np.float32)),
            ...
            if value["path"].endswith("pcm"):
                ...
                if value.get("sampling_rate") is None:
                    ...
                    raise KeyError(
                        "To use PCM files, please specify a 'sampling_rate' in Audio object"
                    )
                if value.get("bytes"):
                    ...
                    bytes_value = (
                        np.memmap(value["path"], dtype="h", mode="r").astype(np.float32)
                        / 32767
                    )

                buffer = BytesIO()
                AudioEncoder(
                    torch.from_numpy(bytes_value), sample_rate=value["sampling_rate"]
                ).to_file_like(buffer, format="wav")
                return {"bytes": buffer.getvalue(), "path": None}
            else:
    ...
    def decode_example(
        ...
        path, bytes = (
            (value["path"], value["bytes"])
            if value["bytes"] is not None
            else (value["path"], None)
        )
        ...
        if bytes is None and is_local_path(path):
            audio = AudioDecoder(
                path, stream_index=self.stream_index, sample_rate=self.sampling_rate
            )
            ...
            audio = AudioDecoder(
                f, stream_index=self.stream_index, sample_rate=self.sampling_rate
            )

        else:
            audio = AudioDecoder(
                bytes, stream_index=self.stream_index, sample_rate=self.sampling_rate
            )
    ...
    def cast_storage(
        ...
        elif pa.types.is_struct(storage.type) and storage.type.get_all_field_indices(
            "array"
        ):
            storage = pa.array(
                [
