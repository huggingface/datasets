...
This guide shows how to process audio data stored in TAR archive - the most frequent case for audio datasets. To get an example of an audio script which uses ZIP archives, check out [minds14](https://huggingface.co/datasets/PolyAI/minds14/blob/main/minds14.py) dataset.
...
1. Use the [`~DownloadManager.download`] method to download metadata file at `_PROMPTS_URLS` and audio TAR archive at `_DATA_URL`. This method returns the path to the local file/archive. In streaming mode, it returns an URL to stream the data from. This method accepts:
...
This implementation does not extract downloaded archives. If you want to extract files after download, you need to additionally use `dl_manager.extract` (see the [(Advanced) Extract TAR archives](#advanced-extract-tar-archives-inside-a-script) section)
How tod it bescribet below
...
TAR files are accessed and yielded sequentially. This means you need to have the information/metadata associated with the audio files in the TAR file in hand first so you can yield it with its corresponding audio file.
...
## (Advanced+) Extract TAR archives inside a script
...
To explain how to do the ectraction, we will briefly go through a [LibriVox Indonesia](https://huggingface.co/datasets/indonesian-nlp/librivox-indonesia) loading script.


### Download and define the dataset splits

1. Use the [`~DownloadManager.download`] method to download the audio data specified in `_AUDIO_URL`.
...
3. Use the [`~DownloadManager.iter_archive`] method to iterate over the archive at `audio_path` after it's downloaded, just as in the Vivos example above. [`~DownloadManager.iter_archive`] doesn't provide any information about the full local paths of files inside the archive, even if they exist locally. As a result, you need to pass the `local_extracted_archive` path to the next step inside `gen_kwargs`, in order to preserve information about where the locally extracted files are. This is required to construct the correct paths to the local files when you generate the examples.
...
4. Use the [`~DownloadManager.download_and_extract`] method to download the metadata file specified in `_METADATA_URL`. If you're not enabling streaming, this method returns a path to a local file. However, if you're planning on enabling streaming, this method opens the file at the URL remotely.
    ...
    While you can use [`~DownloadManager.download_and_extract`] to download and extract TAR archives too, but this method would throw an error in streaming mode.
...
Here `generate_examples` accepts `local_extracted_archive`, `audio_files`, `metadata_path`, and `path_to_clips` from the previous method as arguments.
