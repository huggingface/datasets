...
If you only want to load the underlying path to the image dataset without decoding the image object, pass `decoded=False` to [`~Dataset.set_format`] before indexing the dataset:
