...
If you are using TensorFlow, you can use [`datasets.Dataset.to_tf_dataset`] to wrap the dataset with a *tf.data.Dataset*.
...
means they can be passed directly to methods like *model.fit()*. [`datasets.Dataset.to_tf_dataset`] accepts several arguments:
...
[`datasets.Dataset.to_tf_dataset`] is the easiest way to create a TensorFlow compatible dataset. If you don't want a `tf.data.Dataset` and would rather the dataset emit `tf.Tensor` objects, take a look at the [format](#format) section instead!
