...
3. Upload your files (three methods):

    1. Use 🤗 Datasets `push_to_hub`

    Use the [`push_to_hub()`](https://huggingface.co/docs/datasets/main/en/package_reference/main_classes#datasets.DatasetDict.push_to_hub) function to help you add, commit, and push a file to your repository:
    ...
    2. Use Hugging Face paths in `Pandas`

    Some libraries like [Pandas](https://pandas.pydata.org/), [Dask](https://www.dask.org/) or [DuckDB](https://duckdb.org/) support `hf://` paths to read and upload files:

    ```py
    >>> import pandas as pd

    >>> df = pd.read_csv("hf://datasets/stevhliu/demo/train.csv")
    # df = dataset.apply(...)  # do all your processing here
    >>> df.write_csv("hf://datasets/stevhliu/processed_demo/train.csv")
    ```

    3. Use the `huggingface-cli`

    ```bash
    huggingface-cli upload --repo_ype dataset stevhliu/processed_demo local/path/to/train.csv remote/path/to/train.csv
    ```
...
Load a private dataset by providing your authentication token to the `token` parameter or using `huggingface-cli login`:
