...
There is also a "Batch Processing" option when using the `map` function, which is discussed in the [Map section](#map) above. The `batch` method described here is different and provides a more direct way to create batches from your dataset.

</Tip>

While the `map` function with `batched=True` allows you to apply a function to batches of data, the `batch` method directly transforms your `IterableDataset` into an iterable of batches without applying any function. This is particularly useful when you want to work with batches in your training loop or when using frameworks that expect batched inputs.
