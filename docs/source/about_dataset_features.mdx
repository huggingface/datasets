...
Image datasets have a column with type [`Image`], which load `PIL.Image` objects from images stored as bytes:
...
Depending on the dataset, you may get the path to the local downloaded image, or the content of the image as bytes if the dataset was is not originally made of individual files.
...
For multi-channels arrays like RGB or RGBA, only uint8 is supported. If you use a bigger precision, you get a warning and the array is downcasted to uint8.
For gray-scale images you can use the integer or float precision you want as long as it is compatible with `Pillow`. A warning is shown if your image integer or float precision is too high, and in the case the array is downcated: an int64 array is downcasted to int32, and a float64 array is downcasted to float32. 
