...
attention,None
All the processing methods in this guide return a new [`datasets.Dataset`]. Modification is not done in-place. Be careful about overriding your previous dataset!
...
Use [`datasets.Dataset.sort`] to sort a columns values according to their numerical values. The provided column must be NumPy compatible.
...
The [`datasets.Dataset.shuffle`] method randomly rearranges the values of a column. You can specify the `generator` argument in this method to use a different `numpy.random.Generator` if you want more control over the algorithm used to shuffle the dataset.
...
There are two options for filtering rows in a dataset: [`datasets.Dataset.select`] and [`datasets.Dataset.filter`].

- [`datasets.Dataset.select`] returns rows according to a list of indices:
...
- [`datasets.Dataset.filter`] returns rows that match a specified condition:
...
[`datasets.Dataset.train_test_split`] creates train and test splits, if your dataset doesn't already have them. This allows you to adjust the relative proportions or absolute number of samples in each split. In the example below, use the `test_size` argument to create a test split that is 10% of the original dataset:
...
🤗 Datasets supports sharding to divide a very large dataset into a predefined number of chunks. Specify the `num_shards` argument in [`datasets.Dataset.shard`] to determine the number of shards to split the dataset into. You will also need to provide the shard you want to return with the `index` argument.
...
Use [`datasets.Dataset.rename_column`] when you need to rename a column in your dataset. Features associated with the original column are actually moved under the new column name, instead of just replacing the original column in-place.

Provide [`datasets.Dataset.rename_column`] with the name of the original column, and the new column name:
...
When you need to remove one or more columns, give [`datasets.Dataset.remove_columns`] the name of the column to remove. Remove more than one column by providing a list of column names:
...
[`datasets.Dataset.cast`] changes the feature type of one or more columns. This method takes your new `datasets.Features` as its argument. The following sample code shows how to change the feature types of `datasets.ClassLabel` and `datasets.Value`:
...
Sometimes a column can be a nested structure of several types. Use [`datasets.Dataset.flatten`] to extract the subfields into their own separate columns. Take a look at the nested structure below from the SQuAD dataset:
...
Next, apply this function to the dataset with [`datasets.Dataset.map`]:
...
To disable `tqdm` progress bar printed by default, pass `False` as an argument to [`datasets.set_progress_bar_enabled`].

</Tip>

Let's take a look at another example, except this time, you will remove a column with [`datasets.Dataset.map`]. When you remove a column, it is only removed after the example has been provided to the mapped function. This allows the mapped function to use the content of the columns before they are removed.

Specify the column to remove with the `remove_columns` argument in [`datasets.Dataset.map`]:
...
🤗 Datasets also has a [`datasets.Dataset.remove_columns`] method that is functionally identical, but faster, because it doesn't copy the data of the remaining columns.

</Tip>

You can also use [`datasets.Dataset.map`] with indices if you set `with_indices=True`. The example below adds the index to the beginning of each sentence:
...
Multiprocessing can significantly speed up processing by parallelizing the processes on your CPU. Set the `num_proc` argument in [`datasets.Dataset.map`] to set the number of processes to use:
...
Apply the function with [`datasets.Dataset.map`]:
...
Use [`datasets.Dataset.map`] to apply the function over the whole dataset:
...
Many datasets have splits that you can process simultaneously with [`datasets.DatasetDict.map`]. For example, tokenize the `sentence1` field in the train and test split by:
...
Separate datasets can be concatenated if they share the same column types. Concatenate datasets with [`datasets.concatenate_datasets`]:
...
seealso,None
You can also mix several datasets together by taking alternating examples from each one to create a new dataset. This is known as interleaving, and you can use it with [`datasets.interleave_datasets`]. Both [`datasets.interleave_datasets`] and [`datasets.concatenate_datasets`] will work with regular [`datasets.Dataset`] and [`datasets.IterableDataset`] objects. Refer to the [interleave_datasets](#interleave_datasets) section for an example of how it's used.
...
Set a dataset to a TensorFlow compatible format with [`datasets.Dataset.set_format`]. Specify `type=tensorflow` and the columns that should be formatted:
...
[`datasets.Dataset.with_format`] provides an alternative method to set the format. This method will return a new [`datasets.Dataset`] object with your specified format:
...
Use [`datasets.Dataset.reset_format`] if you need to reset the dataset to the original format:
...
[`datasets.Dataset.set_transform`] allows you to apply a custom formatting transform on-the-fly. This will replace any previously specified format. For example, you can use this method to tokenize and pad tokens on-the-fly:
...
Once you are done processing your dataset, you can save and reuse it later with [`datasets.Dataset.save_to_disk`].
...
When you want to use your dataset again, use [`datasets.load_from_disk`] to reload it:
...
- - Parquet
  - [`datasets.Dataset.to_parquet`]
- - In-memory Python object
  - [`datasets.Dataset.to_pandas`] or [`datasets.Dataset.to_dict`]
