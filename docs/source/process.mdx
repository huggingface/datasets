...
Then you can wrap the dataset with [`~datasets.arrow_dataset.TensorflowDatasetMixin.to_tf_dataset]. This method gives you more control over how to create a [TensorFlow Dataset](https://www.tensorflow.org/api_docs/python/tf/data/Dataset). In the example below, the dataset is created `from_tensor_slices`:
