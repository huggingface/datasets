...
5. When you are done querying, save the index on disk with [`~datasets.search.IndexableMixin.save_faiss_index`]:

```py
>>> ds_with_embeddings.save_faiss_index('embeddings', 'my_index.faiss')
```

6. Reload it at a later time with [`~datasets.search.IndexableMixin.load_faiss_index`]:
...
3. Then you can query the `context` index with [`~datasets.search.IndexableMixin.get_nearest_examples`]:
...
5. Reload it later with the index name when you call [`~datasets.search.IndexableMixin.load_elasticsearch_index`]:
