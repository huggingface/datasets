...
To help you get started with your own loading script, try beginning with a loading script [template](https://github.com/huggingface/datasets/blob/main/templates/new_dataset_script.py).
...
1. Use the [`DownloadManager.download`] method to download the dataset and any other metadata you'd like to associate with it. This function accepts a URL or a list or dictionary of URLs. In the Food-101 loading script, you'll notice again the URLs are defined earlier in the script.
2. After you've downloaded the dataset, use the [`SplitGenerator`] to organize the images and labels in each split. Name each split with a standard name like: `Split.TRAIN`, `Split.TEST`, and `SPLIT.Validation`. 
    ...
    In the `gen_kwargs` parameter, use [`DownloadManager.iter_archive`] to specify the `images` file paths to iterate over and load. You can also specify the associated labels in the `metadata_path`. The `images` and `metadata_path` are actually passed onto the next step where you'll actually generate the dataset.
