...
This returns the `_atom_site` category containing atomic coordinates. The default columns include:
...
```py
>>> dataset = load_dataset("mmcif", data_files="structure.cif", columns=["label_atom_id", "Cartn_x", "Cartn_y", "Cartn_z"])
```
...
Compressed files (`.cif.gz`) are automatically detected and decompressed.
