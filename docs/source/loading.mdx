...
First, create a dataset repository and upload your data files. Then you can use [`datasets.load_dataset`] like you learned in the tutorial. For example, load the files from this [demo repository](https://huggingface.co/datasets/lhoestq/demo1) by providing the repository namespace and dataset name:
...
Refer to the [upload_dataset_repo](#upload_dataset_repo) guide for more instructions on how to create a dataset repository on the Hub, and how to upload your data files.
...
JSON files are loaded directly with [`datasets.load_dataset`] as shown below:
...
🤗 Datasets will also allow you to create a [`datasets.Dataset`] directly from in-memory data structures like Python dictionaries and Pandas DataFrames.

### Python dictionary

Load Python dictionaries with [`datasets.Dataset.from_dict`]:
...
Load Pandas DataFrames with [`datasets.Dataset.from_pandas`]:
...
For even greater control over how to load a split, you can choose to only load specific slices of a split. There are two options for slicing a split: using strings or [`datasets.ReadInstruction`]. Strings are more compact and readable for simple cases, while [`datasets.ReadInstruction`] is easier to use with variable slicing parameters.
...
When you create a dataset from local files, the [`datasets.Features`] are automatically inferred by [Apache Arrow](https://arrow.apache.org/docs/). However, the features of the dataset may not always align with your expectations or you may want to define the features yourself. 

The following example shows how you can add custom labels with [`datasets.ClassLabel`]. First, define your own labels using the [`datasets.Features`] class:
...
Next, specify the `features` argument in [`datasets.load_dataset`] with the features you just created:
...
See the [metric_script](#metric_script) guide for more details on how to write your own metric loading script.
...
3. Load your metric with [`datasets.load_metric`] with these arguments:
...
Once you've loaded a metric for distributed usage, you can compute the metric as usual. Behind the scenes, [`datasets.Metric.compute`] gathers all the predictions and references from the nodes, and computes the final metric.
