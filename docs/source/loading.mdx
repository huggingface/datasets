...
By default, both ATOM and HETATM records are loaded. You can filter to load only specific record types:
...
You can also select specific columns to reduce memory usage:
...
The PDB loader automatically handles gzip, bzip2, and xz compressed files based on magic bytes detection. Extensions `.pdb` and `.ent` are supported.
