...
file adds per-structure metadata. To load mmCIF files from a directory:
...
The `structure` column is parsed from the `_atom_site` category into a struct of
atom-level arrays. The default columns are:
...
| `label_alt_id` | string | Alternate location indicator, or null |
| `label_comp_id` | string | Residue name (e.g., ALA, GLY) |
| `auth_asym_id` | string | Author chain identifier (falls back to `label_asym_id`) |
| `auth_seq_id` | int | Author residue number (falls back to `label_seq_id`) |
| `pdbx_PDB_ins_code` | string | Residue insertion code, or null |
...
| `pdbx_formal_charge` | int | Formal charge, or null |
| `pdbx_PDB_model_num` | int | Model number (1 when the file has a single model) |

You can select specific columns:

```py
>>> dataset = load_dataset("mmcif", data_dir="path/to/structures", columns=["label_atom_id", "Cartn_x", "Cartn_y", "Cartn_z"])
```

To exclude HETATM records (ligands, water molecules):

```py
>>> dataset = load_dataset("mmcif", data_dir="path/to/structures", include_hetatm=False)
```
