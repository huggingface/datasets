...
You can also edit a loading script from the Hub to add your own modifications. Use the [`~datasets.inspect_dataset`] function to download and inspect a loading script:

```py
>>> from datasets import inspect_dataset
>>> script = inspect_dataset("eli5", "local/path/to/datasets/eli5")
```
...
```py
>>> from datasets import load_dataset
>>> eli5 = load_dataset("/local/path/to/datasets/eli5")
```
