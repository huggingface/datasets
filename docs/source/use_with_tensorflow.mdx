...
By default, numeric scalars, lists, and arrays are formatted as `tf.int64` for integers and `tf.float32` for floating-point values, regardless of their stored dtype. This coercion is silent: converting `float64` to `float32` can lose precision or overflow to `inf`.

Pass `dtype=None` to keep the stored numeric dtype, such as `float64` or `int32`:

```py
>>> ds = ds.with_format("tf", dtype=None)
```
