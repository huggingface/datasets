...
In particular, a PyTorch formatted dataset outputs nested lists instead of one single tensor:
...
You can parallelize data loading the `num_workers` argument of a PyTorch `DataLoader` and get a higher throughput.
...
Reloading the dataset inside a worker doesn't fill up your RAM, since it simply memory-map the dataset again from your disk.
...
By default the pytorch data loader load batches of data from a dataset one by one like this:
...
For the pytorch data loader to query batches using a list, you can use a `BatchSampler`:
...
Set the format of a streaming dataset to "pytorch", and it inherits from `torch.utils.data.IterableDataset` so you can pass it to a DataLoader:
