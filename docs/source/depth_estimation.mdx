...
It is mention-worthy that, JPEG/PNG format can only store `uint8` or `uint16` data. As depth map is `float32` data, it can't be stored using PNG/JPEG. However, we can save depth map using TIFF format as it supports a wider range of data types, including `float32` data.
...
It's all black! You'll need to add some color to the depth map to visualize it properly. In order to do that, either we can apply color automatically during display using `plt.imshow()` or create color depth map using `plt.cm` then display it. In this example we have used the latter one, as we can save/write the colored depth map later. (the utility below is taken from the [FastDepth repository](https://github.com/dwofk/fast-depth/blob/master/utils.py)).
...
<div class="flex justify-center">
    <img src="https://i.postimg.cc/nLJLRnHM/depth-est-target-viz.png">
</div>
...
<div class="flex justify-center">
    <img src="https://i.postimg.cc/fLWRs5SN/depth-est-collage.png">
</div>
...
<div class="flex justify-center">
    <img src="https://i.postimg.cc/7hfPG6Nt/depth-est-sample-aug.png">
</div>
...
<div class="flex justify-center">
    <img src="https://i.postimg.cc/N0kjZZMD/depth-est-target-aug.png">
</div>
...
<div class="flex justify-center">
    <img src="https://i.postimg.cc/52v0SXWZ/depth-est-aug-collage.png">
</div>