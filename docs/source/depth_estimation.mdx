...
>>> show_depthmap(train_dataset[index]["depth_map"])
```
...
You can also visualize a collage of different images and the corresponding depth maps.
...
...         train_dataset[idx]["image"], train_dataset[idx]["depth_map"]
...     )
...
```py 
>>> show_depthmap(train_dataset[index]["labels"])
```
