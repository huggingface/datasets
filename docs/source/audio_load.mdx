...
You can also load a dataset with a `AudioFolder` dataset builder. It does not require writing a custom dataloader, making it useful for quickly loading audio data. Your dataset structure might look like this:
...
Load your audio dataset by specifying `audiofolder` and the directory of your dataset in `data_dir`:
...
folder/train/label_9/last_audio_label`_99.mp3
```

`AudioFolder` creates a `label` column of a [`~datasets.features.ClassLabel`] type based on the directory name.

### Language identification

Language identification is one of audio classification tasks for which you might use labels.
...
Load the dataset with `AudioFolder` with `drop_labels=False`, and it will create a `label` column with the language id:
