...
## Batching data in IterableDataset

When working with large datasets, it's often beneficial to process data in batches. 
This is particularly useful for machine learning tasks where models typically expect inputs in batches. 
The `IterableDataset` provides a `batch` method that allows you to easily group samples into batches while maintaining the lazy evaluation nature of the dataset.
...
Batching with `IterableDataset` combines the benefits of processing data in larger chunks with the memory efficiency of streaming data. 
This makes it an excellent choice for working with large datasets that don't fit in memory, especially when used with deep learning frameworks that expect batched inputs.
