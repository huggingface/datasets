...
When you go to load the dataset, `AudioFolder` loads the audio data and creates a `transcription` column containing the text from `metadata.jsonl`:
...
The `data` folder can be any name you want, it doesn't have to be `data`! This folder is optional, unless you're hosting your dataset - audio files or TAR archives - on the Hub.
...
To create different configurations, use the [`BuilderConfig`] class to create a subclass of your dataset. The only required parameter is the `name` of the configuration. Otherwise, you can specify any custom parameters you want in your configuration class. As you can see in this example, the authors also provide a link to download the audio files in `self.data_root_url`.
...
Define your configurations at the top inside [`GeneratorBasedBuilder`], and provide a `name` and short `description` for each configuration:
