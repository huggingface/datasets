...
By default, numeric scalars, lists, and arrays are formatted as `torch.int64` for integers and `torch.float32` for floating-point values, regardless of their stored dtype. This coercion is silent: converting `float64` to `float32` can lose precision or overflow to `inf`.

Pass `dtype=None` to keep the stored floating-point or signed integer dtype, such as `float64` or `int32`:

```py
>>> ds = ds.with_format("torch", dtype=None)
```

Formatting does not change the stored data or feature types.
