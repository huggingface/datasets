...
Stream a dataset by loading it as an [`IterableDataset`]. This allows you to progressively iterate over a dataset without downloading it on disk as well as any local dataset files you have stored.
Learn more about which type of dataset is best for your use case in the (choosing between a regular dataset or an iterable dataset)[./dataset_vs_iterable_dataset] guide.
