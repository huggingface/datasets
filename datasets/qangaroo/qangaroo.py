...
class Qangaroo(nlp.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      QangarooConfig(
          name='medhop',
          description=_MEDHOP_DESCRIPTION,
          data_dir='medhop'
      ),
      QangarooConfig(
          name='masked_medhop',
      ...
      QangarooConfig(
          name='wikihop',
          description=_WIKIHOP_DESCRIPTION,
          data_dir='wikihop'
      ),
      QangarooConfig(
          name='masked_wikihop',
  ...
  def _info(self):
    ...
    return nlp.DatasetInfo(
        ...
        features=nlp.Features({
            ...
            'supports': nlp.features.Sequence({
                'support': nlp.Value('string')
            }),
            'candidates': nlp.features.Sequence({
                'candidate': nlp.Value('string')
            }),
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        nlp.SplitGenerator(
            ...
            gen_kwargs={
                'filepath': os.path.join(data_dir, self.config.data_dir, train_file)
            },
        ),
        nlp.SplitGenerator(
            ...
            # These kwargs will be passed to _generate_examples
            gen_kwargs={
                'filepath': os.path.join(data_dir, self.config.data_dir, dev_file)
            },
        ),
    ]
