...
class Scifact(nlp.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      ScifactConfig(
          name='corpus',
          description=" The corpus of evidence documents"
      ),
      ScifactConfig(
          name='claims',
          description=' The claims are split into train, test, dev'
      )
  ]
  ...
  def _info(self):
    ...
    if self.config.name == 'corpus':
        ...
        features = {
                ...
                "evidence_sentences": nlp.features.Sequence({
                        ...
                        for id1, doc_id in enumerate(evidences):
                            for id2, evidence in enumerate(evidences[doc_id]):
                                yield str(id_)+'_'+str(id1)+'_'+str(id2), {
                                    ...
                                    "evidence_sentences": {
                                        'sentence': evidence['sentences']
                                    },
                                    "cited_doc_ids":{
                                        'doc_id': data.get("cited_doc_ids", [])
                                    }
                                }
                    else:
                        yield id_, {
