...
class FakeNewsEnglish(datasets.GeneratorBasedBuilder):
        ...
        features = datasets.Features(
                {
                    ...
                    "fake_or_satire": datasets.Value("string"),
    ...
    def _generate_examples(self, filepath):
        ...
        #f.apply(lambda x: lib.infer_dtype(x.values))
        #for id_, row in enumerate(f.itertuples()):
        for id_, row in f.iterrows():
            #col1, col2, col3 = row["URL of article"], row["Fake or Satire?"], row["URL of rebutting article"]                   
            #yield str(id_), {"url_of_article": col1, "fake_or_satire": col2, "url_of_rebutting_article": col3} 
            yield id_, {
                    #"article_number": id_ ,
                    "url_of_article": row["URL of article"],
                    "fake_or_satire": row["Fake or Satire?"],
                    "url_of_rebutting_article": row["URL of rebutting article"]
                    }
