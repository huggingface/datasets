...
class KorNLIConfig(nlp.BuilderConfig):
  ...
  def __init__(self,  **kwargs):
    ...
    super(KorNLIConfig, self).__init__(
...
class KorNli(nlp.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      KorNLIConfig(
          name='multi_nli',
          description='Korean multi NLI datasets'
      ),
      KorNLIConfig(
          name='snli',
          description='Korean SNLI dataset'
      ),
      KorNLIConfig(
          name='xnli',
          description='Korean XNLI dataset'
      )
  ]
    ...
    if self.config.name == 'multi_nli':
        return [
            nlp.SplitGenerator(
                ...
                gen_kwargs={
                    'filepath': os.path.join(dl_dir, 'multinli.train.ko.tsv')
                },
            ),
        ]
        ...
        return [
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(dl_dir, 'snli_1.0_train.ko.tsv')
                },
            ),
        ]
    else:
        return [
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(dl_dir, 'xnli.dev.ko.tsv')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.TEST,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(dl_dir, 'xnli.test.ko.tsv')
                },
            ),
        ]
