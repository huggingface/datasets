...
class CocoAnnotationBBoxes(CocoAnnotation):
    ...
    def __init__(self, annotation_path):
        ...
        self._img_id2annotations = {k: list(sorted(v, key=lambda a: a["id"])) for k, v in img_id2annotations.items()}
...
class CocoAnnotationPanoptic(CocoAnnotation):
    ...
    def __init__(self, annotation_path):
        ...
        self._img_id2annotations = {a["image_id"]: a for a in self._data["annotations"]}
...
Split = collections.namedtuple("Split", ["name", "images", "annotations", "annotation_type"])
...
class COCO(datasets.GeneratorBasedBuilder):
            ...
            features.update(
                {
                    ...
                    "panoptic_objects": datasets.Sequence(
                        {
    ...
    def _split_generators(self, dl_manager, BUILDER_CONFIGS=BUILDER_CONFIGS):
        ...
        for split in self.config.split:
            ...
            urls["{}_annotations".format(split.name)] = "annotations/{}.zip".format(split.annotations)
        ...
        return [
            ...
            datasets.SplitGenerator(
                name=datasets.Split.VALIDATION,
                gen_kwargs={"files": dl_manager.iter_archive(archive), "split": "validation"},
            ),
        ]
    ...
    def _generate_examples(self, image_dir, annotation_dir, split_name, annotation_type, panoptic_dir):
        ...
        for image_info in sorted(images, key=lambda x: x["id"]):
            if annotation_type == AnnotationType.BBOXES:
                ...
                #     ]
                # }
                panoptic_annotation = coco_annotation.get_annotations(img_id=image_info["id"])
