  ...
  def _info(self):
    return nlp.DatasetInfo(
        ...
        features=nlp.features.FeaturesDict({
            ...
            'question': {
                'text': nlp.string,
                'tokens': nlp.features.Sequence(nlp.string),
            },
            'annotations': nlp.features.Sequence({
                'id': nlp.string,
                'long_answer': {
                    'start_token': nlp.int64,
                    'end_token': nlp.int64,
                    'start_byte': nlp.int64,
                    'end_byte': nlp.int64,
                },
                ...
                'yes_no_answer': nlp.features.ClassLabel(
  ...
  def _build_pcollection(self, pipeline, filepaths):
    ...
    def _parse_example(line):
      ...
      def _parse_annotation(an_json):
        return {
            # Convert to str since some IDs cannot be represented by nlp.int64.
