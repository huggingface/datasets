...
class Opinosis(nlp.GeneratorBasedBuilder):
  ...
  def _info(self):
    return nlp.DatasetInfo(
        ...
        features=nlp.features.FeaturesDict({
            _REVIEW_SENTS: nlp.string,
            _SUMMARIES: nlp.features.Sequence(nlp.string)
        }),
  ...
  def _generate_examples(self, path=None):
    ...
    filenames = nlp.io.gfile.listdir(topics_path)
    for filename in filenames:
      ...
      summary_lst = []
      for summ_filename in sorted(nlp.io.gfile.listdir(summaries_path)):
