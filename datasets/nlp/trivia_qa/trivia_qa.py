...
def _web_evidence_dir(tmp_dir):
  return nlp.io.gfile.glob(os.path.join(tmp_dir, _WEB_EVIDENCE_DIR))


def _wiki_evidence_dir(tmp_dir):
  return nlp.io.gfile.glob(os.path.join(tmp_dir, _WIKI_EVIDENCE_DIR))
...
class TriviaQA(nlp.GeneratorBasedBuilder):
        ...
        features=nlp.features.FeaturesDict({
            ...
            "answer":
                nlp.features.FeaturesDict({
                    "aliases":
                        nlp.features.Sequence(nlp.string),
                    "normalized_aliases":
                        nlp.features.Sequence(nlp.string),
  ...
  def _split_generators(self, dl_manager):
    ...
    train_files = nlp.io.gfile.glob(os.path.join(qa_dir, _TRAIN_FILE_FORMAT))
    valid_files = nlp.io.gfile.glob(
        os.path.join(qa_dir, _VALIDATION_FILE_FORMAT))
    test_files = nlp.io.gfile.glob(os.path.join(qa_dir, _TEST_FILE_FORMAT))
    ...
    def parse_example(article):
      ...
      def _add_context(collection, context_field, file_dir):
        ...
        for item in collection:
          ...
          except (IOError, nlp.errors.NotFoundError):
      ...
      def _transpose_and_strip_dicts(dicts, field_names):
        return {
            nlp.naming.camelcase_to_snakecase(k):
