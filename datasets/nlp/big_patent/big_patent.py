...
class BigPatentConfig(nlp.BuilderConfig):
  ...
  def __init__(self, cpc_codes=None, **kwargs):
    ...
    super(BigPatentConfig, self).__init__(
        ...
        version=nlp.Version("2.0.0", "Updated to cased raw strings."),
        supported_versions=[nlp.Version("1.0.0")],
        **kwargs)
    self.cpc_codes = cpc_codes


class BigPatent(nlp.BeamBasedBuilder):
  """BigPatent datasets."""
...
def _get_english_words():
  ...
  if not _ENGLISH_WORDS:
    _ENGLISH_WORDS = frozenset(nlp.lazy_imports.nltk.corpus.words.words())
