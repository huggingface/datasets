...
class WmtConfig(nlp.BuilderConfig):
  ...
  def __init__(self,
    ...
    Args:
      ...
      text_encoder_config: `nlp.features.text.TextEncoderConfig` (optional),
  ...
  def _split_generators(self, dl_manager):
    ...
    def _check_manual_files(ds):
      ...
      for fname in manual_dl_files:
        ...
        if not nlp.io.gfile.exists(manual_path):
...
def _parse_parallel_sentences(f1, f2):
  ...
  f1_files = nlp.io.gfile.glob(f1)
  f2_files = nlp.io.gfile.glob(f2)
...
def _parse_czeng(*paths, **kwargs):
  ...
  for path in paths:
    for gz_path in nlp.io.gfile.glob(path):
