  ...
  def __init__(self, text_encoder_config=None, **kwargs):
    ...
    Args:
      text_encoder_config: `nlp.features.text.TextEncoderConfig`, configuration
        for the `nlp.features.text.TextEncoder` used for the CnnDailymail
        (text) features
...
def _find_files(dl_paths, publisher, url_dict):
  ...
  files = nlp.io.gfile.listdir(top_dir)
  ...
  def _split_generators(self, dl_manager):
    ...
    return [
        ...
        nlp.SplitGenerator(
            ...
            gen_kwargs={
                'files': _subset_filenames(dl_paths, nlp.Split.VALIDATION)
            }),
        nlp.SplitGenerator(
            name=nlp.Split.TEST,
            gen_kwargs={'files': _subset_filenames(dl_paths, nlp.Split.TEST)})
    ]
