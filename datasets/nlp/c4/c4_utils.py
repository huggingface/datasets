...
def get_counter_inc_fn(namespace):
    def counter_inc_fn(counter, amt=1):
        nlp.lazy_imports.apache_beam.metrics.Metrics.counter(
...
def get_hashed_url_filter_fn(predicate_fn):
    def filter_fn(el):
        url, _ = el
        val = int(
                hashlib.md5(nlp.compat.as_text(url).encode("utf-8")).hexdigest(), 16)
...
def _get_sentences(text):
    ...
    return list(_SENTENCE_TOKENIZER.tokenize(nlp.compat.as_text(text)))
...
def is_language(page, language, min_probability=0.99):
    ...
    langdetect = nlp.lazy_imports.langdetect
...
def _hash_line(line):
    m = hashlib.md5()
    m.update(nlp.compat.as_text(line).encode("utf-8").strip().lower())
    return m.hexdigest()
...
def _emit_line_to_urls(el, counter_inc_fn):
    ...
    skip_url = min(
            urls,
            key=lambda x: hashlib.md5(nlp.compat.as_text(x).encode("utf-8")).
            hexdigest())
    for url in urls:
        if url != skip_url:
            yield url, line
...
def split_wet_file(wet_file_path, counter_inc_fn=None):
    ...
    with open(wet_file_path, "rb") as f, gzip.GzipFile(
            fileobj=f) as g:
        url = None
        content = None
        content_len = None
...
def is_realnews_domain(el, realnews_domains):
    ...
    url, _ = el
    ext = nlp.lazy_imports.tldextract.extract(url)
