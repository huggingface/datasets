...
class SuperGlue(nlp.GeneratorBasedBuilder):
  ...
  def _info(self):
    ...
    if self.builder_config.name.startswith("wsc"):
      features["span1_index"] = nlp.int32
      features["span2_index"] = nlp.int32
    if self.builder_config.name == "wic":
      features["start1"] = nlp.int32
      features["start2"] = nlp.int32
      features["end1"] = nlp.int32
      features["end2"] = nlp.int32
    if self.builder_config.name == "multirc":
      features["idx"] = nlp.features.FeaturesDict({
          "paragraph": nlp.int32,
          "question": nlp.int32,
          "answer": nlp.int32,
      })
