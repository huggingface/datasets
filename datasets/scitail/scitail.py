    ...
    if self.config.name == 'snli_format':
        return nlp.DatasetInfo(
            ...
            features=nlp.Features({
                ...
                'annotator_labels': nlp.features.Sequence({
                    'annotator_label': nlp.Value('string')
                }
                ),
    ...
    if self.config.name == 'snli_format':
        return [
            nlp.SplitGenerator(
                ...
                gen_kwargs={
                    'filepath': os.path.join(snli, 'scitail_1.0_train.txt')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.TEST,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(snli, 'scitail_1.0_test.txt')
                },
            ),
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(snli, 'scitail_1.0_dev.txt')
                },
            ),
        ]
    elif self.config.name == 'tsv_format':
        return [
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(tsv, 'scitail_1.0_train.tsv')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.TEST,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(tsv, 'scitail_1.0_test.tsv')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.VALIDATION,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(tsv, 'scitail_1.0_dev.tsv')
                },
            ),
        ]
    elif self.config.name == 'predictor_format':
        return [
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(predictor, 'scitail_1.0_structure_train.jsonl')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.TEST,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(predictor, 'scitail_1.0_structure_test.jsonl')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.VALIDATION,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(predictor, 'scitail_1.0_structure_dev.jsonl')
                },
            ),
        ]
    elif self.config.name == 'dgem_format':
        return [
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(dgem, 'scitail_1.0_structure_train.tsv')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.TEST,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(dgem, 'scitail_1.0_structure_test.tsv')
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.VALIDATION,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': os.path.join(dgem, 'scitail_1.0_structure_dev.tsv')
                },
            ),
        ]
        ...
        if self.config.name == 'snli_format':
            for id_, row in enumerate(f):
                ...
                yield id_, {
                    ...
                    'annotator_labels': {
                        'annotator_label': data['annotator_labels']
                    }
                    ,
