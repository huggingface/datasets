...
url = {https://hdl.handle.net/11234/1-3424},
...
class UniversaldependenciesConfig(datasets.BuilderConfig):
    ...
    def __init__(self, data_url, **kwargs):
        super().__init__(version=datasets.Version("2.7.0", ""), **kwargs)
        ...
        for path in filepath:
            with open(path, encoding="utf-8") as data_file:
                for sent in conllu.parse_incr(data_file):
                    idx = sent.metadata.get("sent_id", id_)
                    txt = sent.metadata.get("text", " ".join(token["form"] for token in sent))
                    yield id_, {
                        "idx": str(idx),
                        "text": txt,
