...
(Please also see the README of the MovieLens data, which this data is partly based on:
http://files.grouplens.org/datasets/movielens/ml-20m-README.html,
and the license for the OMDB data, which this data is partly based on:
...
class Mdd(datasets.GeneratorBasedBuilder):
    ...
    DEFAULT_CONFIG_NAME = (
        "task1_qa"  # It's not mandatory to have a default configuration. Just use one if it make sense.
    )
        ...
        features = datasets.Features(
            {
                "dialogue_turns": datasets.Sequence(
                    {
                        "speaker": datasets.ClassLabel(names=["0", "1"]),
