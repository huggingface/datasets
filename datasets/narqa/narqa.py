...
_DESCRIPTION = """\
The Narrative QA dataset is a reading comprehension \
...
You need to manually download the stories for this dataset using the script provided by the authors \
...
can be used to load the dataset via `datasets.load_dataset("narqa", data_dir="<path/to/folder>").                """
...
class NarrativeQA(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if not os.path.exists(path_to_manual_folder):
            raise FileNotFoundError(
                "{} does not exist. Make sure you insert a manual dir via `datasets.load_dataset('narqa', data_dir=...)` that includes the stories downloaded from the original repository. Manual download instructions: {}".format(
    ...
    def _generate_examples(self, filepath, split):
        ...
        for keys in data:
            yield keys, {
                "document_id": data[keys]["document_id"],
                "kind": data[keys]["kind"],
