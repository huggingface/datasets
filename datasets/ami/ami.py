...
class AMI(datasets.GeneratorBasedBuilder):
    ...
    def _info(self):
        ...
        if self.config.name == "headset-single":
            ...
            features_dict.update({f"audio-{i}": datasets.Audio(sampling_rate=16_000) for i in range(4)})
            config_description = (
            ...
            features_dict.update({f"audio-1-{i}": datasets.Audio(sampling_rate=16_000) for i in range(1, 8)})
            config_description = (
    ...
    def _generate_examples(self, annotation_path, samples_paths, ids, verification_ids):
        ...
        for _id in words_paths.keys():
            ...
            if self.config.name in ["headset-single", "microphone-single"]:
                result.update({"audio": samples_paths_dict[_id][0]})
