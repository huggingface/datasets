...
class Tydiqa(nlp.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      ...
      TydiqaConfig(
          name='secondary_task',
          description=textwrap.dedent("""\Gold passage task (GoldP): Given a passage that is guaranteed to contain the 
        ...
        return nlp.DatasetInfo(
            ...
            features=nlp.Features({
                'passage_answer_candidates': nlp.features.Sequence({
                    'plaintext_start_byte': nlp.Value('int32'),
                    'plaintext_end_byte': nlp.Value('int32')
                }),
    ...
    if self.config.name == 'primary_task':
        return [
            nlp.SplitGenerator(
                ...
                gen_kwargs={
                    'filepath': primary_downloaded['train']
                },
            ),
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': primary_downloaded['dev']
                },
            ),
        ]
    elif self.config.name == 'secondary_task':
        return [
            nlp.SplitGenerator(
                ...
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': secondary_downloaded['train']
                },
            ),
            nlp.SplitGenerator(
                name=nlp.Split.VALIDATION,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={
                    'filepath': secondary_downloaded['dev']
                },
            ),
        ]
    ...
    if self.config.name == 'primary_task':
        with open(filepath) as f:
            for id_, row in enumerate(f):
                ...
                min_answers_end_byte = [annotation['minimal_answer']['plaintext_end_byte'] for annotation in
                                        annotations]
                min_answers_start_byte = [annotation['minimal_answer']['plaintext_start_byte'] for annotation in
                                          annotations]
