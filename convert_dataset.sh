...
pathToFolder="datasets/nlp/${datasetName}"
...
if [ -f "${pathToFolder}/${datasetName}.py" ]; then
	...
	eval "python nlp-cli convert --tfds_path ${pathToFile} --nlp_directory datasets/nlp"
fi
...
if [ -f ${curPath}/${pathToFolder}/dummy/${firstVersion}/dummy_data.zip ] || [ -f ${curPath}/${pathToFolder}/dummy/${firstConfig}/${firstVersion}/dummy_data.zip ] ; then
	...
	echo ""
	echo ""
	echo "Now you can upload the new dataset folder by running: "
	echo "aws s3 cp datasets/nlp/${datasetName} s3://datasets.huggingface.co/nlp/${datasetName} --recursive"
	echo ""
	echo ""
	echo "After upload check that the following two commands work:"
	echo "RUN_SLOW=1 pytest tests/test_dataset_common.py::DatasetTest::test_load_real_dataset_${datasetName}"
echo "RUN_SLOW=1 pytest tests/test_dataset_common.py::DatasetTest::test_load_dataset_all_configs_${datasetName}"
echo "pytest tests/test_dataset_common.py::DatasetTest::test_load_dataset_${datasetName}"
fi
