...
class TestPushToHubZarr:
    ...
    def test_large_store_warns_and_uploads_unchanged(self, tmp_path, capsys):
        from unittest.mock import MagicMock, patch
        ...
        with patch("huggingface_hub.HfApi") as mock_api_cls, patch(
            ...
            mock_api = MagicMock()
            mock_api.upload_folder.return_value = "https://huggingface.co/datasets/test/repo"
            mock_api_cls.return_value = mock_api
    ...
    def test_bucket_destination_existing_bucket(self, tmp_path):
        from unittest.mock import MagicMock, patch
        ...
        with patch("huggingface_hub.HfApi") as mock_api_cls:
            mock_api = MagicMock()
            mock_api.bucket_info.return_value.id = "user/mybucket"
            mock_api_cls.return_value = mock_api

            url = push_to_hub_zarr(str(src), repo_id="buckets/user/mybucket")
            assert url == "hf://buckets/user/mybucket/store.zarr"
            mock_api.bucket_info.assert_called_once()
            mock_api.create_bucket.assert_not_called()
            mock_api.sync_bucket.assert_called_once()
            call_kwargs = mock_api.sync_bucket.call_args[1]
            assert call_kwargs["source"] == str(src)
            assert call_kwargs["dest"] == "hf://buckets/user/mybucket/store.zarr"

    def test_bucket_destination_creates_bucket(self, tmp_path):
        from unittest.mock import MagicMock, patch
        ...
        with patch("huggingface_hub.HfApi") as mock_api_cls:
            mock_api = MagicMock()
            mock_api.bucket_info.side_effect = BucketNotFoundError(
                "bucket not found", response=MagicMock()
            )
            mock_bucket_url = MagicMock()
            mock_bucket_url.bucket_id = "user/mybucket"
            mock_api.create_bucket.return_value = mock_bucket_url
            mock_api_cls.return_value = mock_api

            url = push_to_hub_zarr(str(src), repo_id="buckets/user/mybucket", private=True)
            assert url == "hf://buckets/user/mybucket/store.zarr"
            mock_api.create_bucket.assert_called_once()
            create_kwargs = mock_api.create_bucket.call_args[1]
            assert create_kwargs["private"] is True
            assert create_kwargs["exist_ok"] is True

    def test_bucket_destination_with_subpath(self, tmp_path):
        from unittest.mock import MagicMock, patch
        ...
        with patch("huggingface_hub.HfApi") as mock_api_cls:
            ...
            url = push_to_hub_zarr(
                str(src),
                repo_id="buckets/user/mybucket/raw/",
                path_in_repo="img",
            )
            assert url == "hf://buckets/user/mybucket/raw/img"
            call_kwargs = mock_api.sync_bucket.call_args[1]
            assert call_kwargs["dest"] == "hf://buckets/user/mybucket/raw/img"
