    ...
    def _split_generators(self, dl_manager, pipeline):
        return [
            datasets.SplitGenerator(
                ...
                gen_kwargs={"examples": get_test_dummy_examples()},
            )
        ]
...
class NestedBeamDataset(datasets.BeamBasedBuilder):
    ...
    def _info(self):
        return datasets.DatasetInfo(
            features=datasets.Features(
                {"a": datasets.Sequence({"b": datasets.Value("string")})}
            ),
            # No default supervised_keys.
            supervised_keys=None,
        )
    ...
    def _split_generators(self, dl_manager, pipeline):
        return [
            datasets.SplitGenerator(
                name=datasets.Split.TRAIN,
                gen_kwargs={"examples": get_test_nested_examples()},
            )
        ]
...
def get_test_dummy_examples():
    return [
        (i, {"content": content}) for i, content in enumerate(["foo", "bar", "foobar"])
    ]
...
def get_test_nested_examples():
    return [
        (i, {"a": {"b": [content]}})
        for i, content in enumerate(["foo", "bar", "foobar"])
    ]
...
class BeamBuilderTest(TestCase):
    ...
    def test_download_and_prepare(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_cache_dir:
            builder = DummyBeamDataset(
                cache_dir=tmp_cache_dir, beam_runner="DirectRunner"
            )
            ...
            self.assertTrue(
                os.path.exists(
                    os.path.join(
                        ...
                        "default",
                        "0.0.0",
                        "dummy_beam_dataset-train.arrow",
                    )
                )
            )
            self.assertDictEqual(
                builder.info.features,
                datasets.Features({"content": datasets.Value("string")}),
            )
            dset = builder.as_dataset()
            self.assertEqual(dset["train"].num_rows, expected_num_examples)
            self.assertEqual(
                dset["train"].info.splits["train"].num_examples, expected_num_examples
            )
            self.assertDictEqual(dset["train"][0], get_test_dummy_examples()[0][1])
            self.assertDictEqual(
                dset["train"][expected_num_examples - 1],
                get_test_dummy_examples()[expected_num_examples - 1][1],
            )
            self.assertTrue(
                os.path.exists(
                    os.path.join(
    ...
    def test_no_beam_options(self):
        with tempfile.TemporaryDirectory() as tmp_cache_dir:
            ...
            self.assertRaises(
                datasets.builder.MissingBeamOptions, builder.download_and_prepare
            )
