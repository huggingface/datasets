...
from datasets.commands.dummy_data import (DummyDataGeneratorDownloadManager,
                                          MockDownloadManager)
...
class DummyBuilder(GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        return [
            SplitGenerator(
                Split.TRAIN, gen_kwargs={"filepath": downloaded_files["train"]}
            ),
            SplitGenerator(
                Split.TEST, gen_kwargs={"filepath": downloaded_files["test"]}
            ),
        ]
    ...
    def test_dummy_data_autogenerate(self):
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            download_config = DownloadConfig(
                cache_dir=os.path.join(tmp_dir, datasets.config.DOWNLOADED_DATASETS_DIR)
            )
            ...
            dataset_builder.download_and_prepare(
                dl_manager=dl_manager, try_from_hf_gcs=False
            )
...
def test_create_xml_dummy_data(xml_file, tmp_path):
    ...
    DummyDataGeneratorDownloadManager._create_xml_dummy_data(
        xml_file, dst_path, "tu", n_lines=1
    )
