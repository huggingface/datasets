...
@pytest.fixture
def hub_dataset_info_patterns_results(
    hub_dataset_info, complex_data_dir, pattern_results
):
    return {
        pattern: [
            hf_hub_url(
                hub_dataset_info.id,
                Path(path).relative_to(complex_data_dir).as_posix(),
                revision=hub_dataset_info.sha,
            )
...
@pytest.mark.parametrize("pattern", _TEST_PATTERNS)
def test_resolve_patterns_locally_or_by_urls(
    complex_data_dir, pattern, pattern_results
):
    resolved_data_files = resolve_patterns_locally_or_by_urls(
        complex_data_dir, [pattern]
    )
...
@pytest.mark.parametrize(
    ...
    [("*", 2, ["txt"]), ("*", 2, None), ("*", 0, ["blablabla"])],
)
def test_resolve_patterns_locally_or_by_urls_with_extensions(
...
@pytest.mark.parametrize("pattern", _TEST_PATTERNS)
def test_resolve_patterns_in_dataset_repository(
    hub_dataset_info, pattern, hub_dataset_info_patterns_results
):
    resolved_data_files = resolve_patterns_in_dataset_repository(
        hub_dataset_info, [pattern]
    )
    assert (
        sorted(str(f) for f in resolved_data_files)
        == hub_dataset_info_patterns_results[pattern]
    )
...
def test_resolve_patterns_in_dataset_repository_with_extensions(
    hub_dataset_info, pattern, size, extensions
):
    if size > 0:
        resolved_data_files = resolve_patterns_in_dataset_repository(
...
def test_resolve_patterns_in_dataset_repository_sorted_files():
    ...
    datasets_infos = DatasetInfo(
        id="test_unsorted_files", siblings=siblings, sha="foobar"
    )
...
def test_DataFilesList_from_hf_repo(
    hub_dataset_info, hub_dataset_info_patterns_results, pattern
):
    data_files_list = DataFilesList.from_hf_repo([pattern], hub_dataset_info)
    assert (
        sorted(str(f) for f in data_files_list)
        == hub_dataset_info_patterns_results[pattern]
    )
...
def test_DataFilesDict_from_hf_repo(
    ...
    assert (
        sorted(str(f) for f in data_files[split_name])
        == hub_dataset_info_patterns_results[pattern]
    )
