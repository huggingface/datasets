...
```
config = nlp.translate.wmt.WmtConfig(
    version="0.0.1",
    language_pair=("fr", "de"),
    subsets={
...
<<<<<<< This should probably be modified because it mentions: ['encoder_config']
...
<<<<<<< This should probably be modified because it mentions: ['TextEncoderConfig', 'encoder_config']
...
class WmtTranslate(nlp.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager):
    ...
    def _check_manual_files(ds):
      ...
      for fname in manual_dl_files:
        ...
        if not nlp.Value('io').gfile.exists(manual_path):
...
<<<<<<< This should probably be modified because it mentions: ['SubwordTextEncoder']
    # Generate vocabulary from training data if SubwordTextEncoder configured.
=======
>>>>>>>
    for language in self.builder_config.language_pair:
<<<<<<< This should probably be modified because it mentions: ['maybe_build_from_corpus']
      self.info.features[language].maybe_build_from_corpus(
=======
>>>>>>>
          self._vocab_text_gen(
              self.subsets[nlp.Split.TRAIN], extraction_map, language))
